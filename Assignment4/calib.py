...
def calibrate2D(ref3D, ref2D) :
    ...
    pxz = lstsq(Axz, bxz, rcond=None)[0]
    Hxz_ = np.append(pxz, 1)
    Hxz = Hxz_.reshape((3, 3))

    # TODO : form the matrix equation Ap = b for the Y-Z plane
    #create an original Ayz
    byz = np.zeros((8, 1))
    ...
    # TODO : solve for the planar projective transformation using linear least squares
    pyz = lstsq(Ayz, byz, rcond=None)[0]
    Hyz_ = np.append(pyz, 1)
    Hyz = Hyz_.reshape((3, 3))
...
def gen_correspondences(Hxz, Hyz, corners) :
    ...
    # TODO : define 3D coordinates of all the corners on the 2 calibration plane
    co3Dxz_ = np.zeros((80, 3))
    co3Dyz_ = np.zeros((80, 3))
    for m in range(10):
        for n in range(8):
            coX = 0.5+m
            coY = 0.5+n
            i = n*10+m
            co3Dxz = np.array([coX, 0, coY])
            co3Dyz = np.array([0, coX, coY])
            co3Dxz_[i] = co3Dxz
            co3Dyz_[i] = co3Dyz

    ref3D = np.concatenate((co3Dxz_, co3Dyz_))

    # TODO : project corners on the calibration plane 1 onto the image
    bxz_2D = []
    
    for k in range(160):
        if((ref3D[k][0] >= 0) and (ref3D[k][2] >= 0) and (ref3D[k][0] <= 10) and (ref3D[k][2] <= 8) and (ref3D[k][1] == 0)):
            Pxz = np.array([ref3D[k][0],ref3D[k][2],1])
            bxz = Hxz@Pxz
            bxz /= bxz[2]
            bxz_2D.append(bxz[0:2])

    bxz_2D_array = np.array(bxz_2D)

    # TODO : project corners on the calibration plane 2 onto the image
    byz_2D = []
    for k in range(160):
        if((ref3D[k][1] >= 0) and (ref3D[k][2] >= 0) and (ref3D[k][1] <= 10) and (ref3D[k][2] <= 8) and (ref3D[k][0]== 0)):
            Pyz = np.array([ref3D[k][1],ref3D[k][2],1])
            byz = Hyz@Pyz
            byz /= byz[2]
            byz_2D.append(byz[0:2])

    byz_2D_array = np.array(byz_2D)

    # TODO : locate the nearest detected corners
    b_2D = np.concatenate((bxz_2D_array, byz_2D_array))
    ref2D = find_nearest_corner(b_2D, corners)
...
def calibrate3D(ref3D, ref2D) :
    ...
    b = np.zeros((len(ref2D)*2, 1))
    A = np.zeros((len(ref3D)*2, 11))
...
def pick_corners(img_color, corners) :
    ...
    # return:
    #    ref3D - a 8 x 3 numpy ndarray holding the 3D coordinates of the extreme
    #            corners on the 2 calibration planes
    #    ref2D - a 8 x 2 numpy ndarray holding the 2D coordinates of the projections
    #            of the corners in ref3D
